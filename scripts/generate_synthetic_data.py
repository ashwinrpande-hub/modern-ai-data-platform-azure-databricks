...
def sap():
    ...
    with open("data/sap_vbak.csv","w",newline="") as f:
        ...
        for i in range(5000):
            vbeln, amt, d = f"00{4000000+i}", round(random.uniform(5e3,9e5),2), _date()
            orders.append((vbeln, amt, d))
            w.writerow([vbeln, f"EU{random.randint(1,500):05d}", amt,
                random.choice(["EUR","GBP"]), d.isoformat(), random.choice(STEEL)])
    ...
    # DV2.0 bridge lifecycle: ~70% of orders ship (LIPS), ~80% of shipped invoice (VBRP)
    with open("data/sap_lips.csv","w",newline="") as fl, open("data/sap_vbrp.csv","w",newline="") as fi:
        wl=csv.writer(fl); wl.writerow(["VBELN","POSNR","VGBEL","LFIMG","ERDAT","WERKS"])
        wi=csv.writer(fi); wi.writerow(["VBELN","POSNR","AUBEL","VGBEL","NETWR","ERDAT"])
        for n,(vbeln, amt, d) in enumerate(orders):
            if random.random() >= 0.70: continue
            dlv, ship_d = f"008{5000000+n}", d+dt.timedelta(days=random.randint(3,20))
            wl.writerow([dlv, "000010", vbeln, random.randint(1,40), ship_d.isoformat(), random.choice(["NC01","AL02","TX03"])])
            if random.random() < 0.80:
                inv_d = ship_d+dt.timedelta(days=random.randint(2,15))
                wi.writerow([f"009{6000000+n}", "000010", vbeln, dlv, amt, inv_d.isoformat()])
