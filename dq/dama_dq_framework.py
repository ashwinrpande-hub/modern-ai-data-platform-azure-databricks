"""
DAMA 6-dimension DQ framework. Rules live in nucor_bronze.cfg.layer_mappings.dq_rule
...
Outputs: nucor_bronze.audit.dq_results (scores) + rejected_records (row-level).
"""
...
RULES = [  # (table, dimension, rule_sql, threshold)
    ("nucor_silver.sales.sales_order_header", "COMPLETENESS", "src_customer_id IS NOT NULL", 0.99),
    ("nucor_silver.sales.sales_order_header", "VALIDITY",     "order_amount_usd BETWEEN 0 AND 50000000", 0.999),
    ("nucor_silver.sales.sales_order_header", "UNIQUENESS",   None, 0.999),   # dup hk+effective_ts
    ("nucor_silver.sales.sales_order_header", "TIMELINESS",   "effective_ts >= current_timestamp() - INTERVAL 1 DAY", 0.95),
    ("nucor_silver.sales.customer",           "CONSISTENCY",  "country RLIKE '^[A-Z]{2}$'", 0.98),
    ("nucor_gold.sales.fact_sales_orders",    "ACCURACY",     "customer_hk IS NOT NULL", 0.99),
]
...
def run():
    ...
    ).write.mode("append").saveAsTable("nucor_bronze.audit.dq_results")
