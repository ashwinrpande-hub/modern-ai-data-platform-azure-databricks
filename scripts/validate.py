...
CHECKS = [
 ...
 ("PIT has today's snapshot",  "SELECT count(*) FROM nucor_gold.sales.pit_customer WHERE snapshot_date = current_date()", ">=1"),
 ("Bridge FK integrity (orders)", "SELECT count(*) FROM nucor_gold.sales.bridge_order_fulfillment b LEFT ANTI JOIN nucor_gold.sales.fact_sales_orders f ON b.order_hk=f.order_hk", 0),
]
